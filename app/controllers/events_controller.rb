class EventsController < ApplicationController
  ...
  def create
    ...
    if @event.save
      ...
      render 'new', notice: @event.errors.full_message.first
    end
  end
  ...
  def calendar
    if request.xhr?
      friend_events = Event.select("events.*").joins("INNER JOIN follows ON events.user_id = follows.followable_id").where("follows.follower_id = #{current_user.id} AND follows.followabe_type = 'User'")
